...
@objc public protocol GooglePlacesAutocompleteDelegate {
    ...
    optional func placeViewCommit()
    optional func placeViewClosed()
    optional func placeViewReset()
}
...
public class GooglePlacesAutocomplete: UINavigationController {
    ...
    public convenience init(apiKey: String, placeType: PlaceType = .All) {
        ...
        closeButton = UIBarButtonItem(
            ...
            target: self,
            action: #selector(GooglePlacesAutocomplete.close))
        closeButton.style = UIBarButtonItemStyle.Done
        
        gpaViewController.navigationItem.leftBarButtonItem = closeButton
        gpaViewController.navigationItem.title = "Enter Address"
        gpaViewController.navigationController?.navigationBarHidden = true
    }
...
public class GooglePlacesAutocompleteContainer: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        NSNotificationCenter.defaultCenter()
            .addObserver(self,
                         selector: #selector(GooglePlacesAutocompleteContainer.keyboardWasShown(_:)),
                         name: UIKeyboardDidShowNotification, object: nil)
        
        NSNotificationCenter.defaultCenter()
            .addObserver(self,
                         selector: #selector(GooglePlacesAutocompleteContainer.keyboardWillBeHidden(_:)),
    ...
    func setBottomButtonIcon() {
     
        let buttonString = String.fontAwesomeIconWithName(FontAwesome.LocationArrow)
        ...
        myLocationButton.titleLabel?.textAlignment = .Center
        myLocationButton.titleLabel?.numberOfLines = 2
        myLocationButton.setAttributedTitle(buttonStringAttributed, forState: .Normal)
    }
    ...
    @IBAction func commitAction(sender: AnyObject) {
        ...
        delegate?.placeViewCommit?()
    }
    
    @IBAction func useMyLocationAction(sender: AnyObject) {
        
        self.searchBar.text = ""
        self.places.removeAll()
        self.tableView.reloadData()
        
        delegate?.placeViewReset?()
    }
...
extension GooglePlacesAutocompleteContainer: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(tableView: UITableView,
        ...
        cell.selectedBackgroundView?.backgroundColor = UIColor(rgb: 0x51e2d4)
